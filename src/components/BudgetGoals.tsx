...
export default function BudgetGoals({ budgets, progress, onSave, onDelete }: Props) {
  ...
  return (
    <div className="flex flex-col gap-8">
      ...
      <div
        className="rounded-[20px] p-5 sm:p-6 transition-all shadow-2xs"
        style={{
          background: "var(--shell-card-solid)",
          border: "1px solid var(--shell-border)",
        }}
      >
        <div className="flex flex-wrap items-center justify-between gap-3 mb-4 pb-3 border-b" style={{ borderColor: "var(--shell-border)" }}>
          <div>
            <h3 className="text-sm font-bold text-zinc-900 dark:text-zinc-100 flex items-center gap-2">
              <span>🎯</span>
              <span>Kategoriye Göre Bütçe Hedefi Belirle</span>
            </h3>
            <p className="text-xs text-zinc-500 mt-0.5">
              Limit belirlemek istediğiniz kategorinin yanına aylık azami harcama hedefinizi yazın.
            </p>
          </div>
          <span className="text-xs font-semibold px-2.5 py-1 rounded-full" style={{ background: "oklch(0.5 0.02 50 / 0.1)", color: "var(--shell-muted)" }}>
            {budgets.length} / {EXPENSE_CATEGORIES.length} Kategori
          </span>
        </div>

        {availableCategories.length === 0 ? (
          <div className="flex items-center gap-2.5 text-xs font-medium py-2" style={{ color: "var(--shell-positive)" }}>
            <span>✨</span>
            <span>Harika! Tüm kategorileriniz için aylık bütçe hedefi tanımlanmış durumda.</span>
          </div>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3.5">
            {availableCategories.map((category) => {
              ...
              return (
                <div
                  key={category}
                  className="flex items-center gap-3 rounded-2xl p-2.5 transition-all hover:shadow-2xs"
                  style={{
                ...
                >
                  {imgSrc ? (
                    <div className="relative h-11 w-11 shrink-0 overflow-hidden rounded-xl border border-zinc-200/80 dark:border-zinc-700/80 shadow-2xs">
                      <Image src={imgSrc} alt={category} fill className="object-cover" />
                    </div>
                  ) : (
                    <div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl bg-amber-500/10 text-base">
                      📁
                    </div>
                  )}
                    ...
                    <div className="mt-1 flex items-center gap-1.5">
                      <div className="relative flex-1">
                        <input
                          ...
                          className="w-full rounded-lg px-2.5 py-1 text-xs font-medium outline-none transition-all"
                          style={{
                            background: "var(--shell-card-solid)",
                            border: "1px solid var(--shell-border)",
                      ...
                      <button
                        ...
                        className="rounded-lg px-3 py-1 text-xs font-semibold text-white transition-all disabled:opacity-40 hover:opacity-90 active:scale-95 shrink-0 shadow-2xs"
      ...
      {budgets.length === 0 ? (
        <div
          className="flex flex-col items-center justify-center gap-3 rounded-[22px] border border-dashed p-10 text-center"
        ...
        >
          ...
          <div>
            <h4 className="font-bold text-base text-zinc-900 dark:text-zinc-100">Henüz Aktif Bütçe Hedefi Yok</h4>
            <p className="text-xs text-zinc-500 mt-1 max-w-md">
              Yukarıdaki kategorilerden dilediğinize aylık harcama limiti belirleyin. Böylece her dönem bütçenizi aşıp aşmadığınızı kedi temalı göstergelerle takip edebilirsiniz.
            </p>
          </div>
        </div>
      ) : (
            ...
            return (
              ...
              >
                ...
                {imgSrc ? (
                  ...
                  <div className="flex h-32 w-full sm:w-32 shrink-0 items-center justify-center rounded-2xl bg-amber-500/10 text-4xl shadow-2xs">
                    🐱
                  </div>
                )}
                ...
                <div className="flex flex-1 flex-col justify-between gap-3 min-w-0">
                  ...
                  <div className="flex items-center justify-between gap-2">
                    <div className="flex items-center gap-2">
                      ...
                      >
                        {over ? "AŞILDI" : `%${p ? p.percentUsed.toFixed(0) : 0}`}
                      </span>
                    </div>
                  ...
                  <div className="flex flex-wrap items-center justify-between gap-1.5 text-[11px] pt-0.5">
                    <span className="font-semibold" style={{ color: over ? "var(--shell-negative)" : "var(--shell-muted)" }}>
                      {over
                        ? `⚠️ Hedef ${formatTRY(spent - b.monthlyGoal)} aşıldı`
                        : `Kalan Bütçe: ${formatTRY(remaining)}`}
                    </span>
                    ...
                    {p && p.lastMonthTotal > 0 && (
                      ...
                      >
                        <span>{delta > 0 ? "↑" : "↓"}</span>
                        <span>Geçen döneme göre {delta > 0 ? "+" : ""}{formatTRY(delta)}</span>
                      </span>
                    )}
                  </div>
                </div>
